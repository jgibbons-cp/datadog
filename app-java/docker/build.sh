...
if [ "$OTEL" = 1 ]; then
  ...
  docker buildx build --platform linux/amd64,linux/arm64 --push -t jenksgibbons/app-java -f docker/Dockerfile.tomcat --build-arg TRACER=otel-java-tracer.jar .
elif [ "$DD_TRACER" = 1 ]; then
  wget -O dd-java-tracer.jar https://dtdg.co/latest-java-tracer
  docker buildx build --platform linux/amd64,linux/arm64 --push -t jenksgibbons/app-java -f docker/Dockerfile.tomcat --build-arg TRACER=dd-java-tracer.jar .
else
  docker buildx build --platform linux/amd64,linux/arm64 --push -t jenksgibbons/app-java -f docker/Dockerfile.tomcat --build-arg TRACER= .
fi
